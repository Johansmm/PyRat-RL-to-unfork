...
# Sound effects. There are three sounds corresponding to any combination of players taking pieces
# of cheese at a given moment
try:
    if not(args.nodrawing) and not(args.save_images):
        pygame.mixer.init(frequency=44100, size=-16, channels=1, buffer=2**12)
        effect_left = pygame.mixer.Sound(
            "resources" + os.path.sep + "sounds" + os.path.sep + "cheese_left.wav")
        effect_right = pygame.mixer.Sound(
            "resources" + os.path.sep + "sounds" + os.path.sep + "cheese_right.wav")
        effect_both = pygame.mixer.Sound(
            "resources" + os.path.sep + "sounds" + os.path.sep + "cheese_both.wav")
        nosound = False
    else:
...
def player(pet, filename, q_in, q_out, q_quit, width, height, preparation_time, turn_time):
    ...
    # In case there is a problem, we launch the dummy AI which basically does nothing
        ...
        if filename != "":
            ...
            print("Error while loading player controlling " + pet +
                  ", dummy player loaded instead", file=sys.stderr)
        player = importlib.util.spec_from_file_location(
            "player", "resources" + os.path.sep + "imports" + os.path.sep + "dummy_player.py")
        module = importlib.util.module_from_spec(player)
        player.loader.exec_module(module)
        existence = False
    ...
    try:
        before = time.time()
        preprocessing(maze, width, height, player1_location,
                      player2_location, pieces_of_cheese, preparation_time)
        after = time.time()
        ...
        while 1:
            ...
            try:
                ...
                decision = turn(maze, width, height, player1_location,
                                player2_location, score1, score2, pieces_of_cheese, turn_time)
                after = time.time()
...
def convert_time_to_int(datetime):
    return datetime.hour * 3600000 + datetime.minute * 60000 + datetime.second * 1000\
...
def move(decision1, decision2, maze, player1_location, player2_location, stuck1, stuck2,
         moves1, moves2, miss1, miss2):
...
def run_game(screen, infoObject):
    ...
    # Load saved match
    if args.load_match:
        args.rat = args.load_match + os.path.sep + \
            [f for f in os.listdir(args.load_match) if f[:4] == "rat_"][0]
        args.python = args.load_match + os.path.sep + \
            [f for f in os.listdir(args.load_match) if f[:7] == "python_"][0]
    ...
    width, height, pieces_of_cheese, maze, player1_location, player2_location = generate_maze(
        args.width, args.height, args.density,
        not(args.nonconnected), not(args.nonsymmetric),
        args.mud_density, args.mud_range,
        args.maze_file, random_seed
    )
    if args.maze_file:
    ...
    if pieces_of_cheese == []:
        pieces_of_cheese, player1_location, player2_location = generate_pieces_of_cheese(
            args.pieces, width, height, not(args.nonsymmetric), player1_location, player2_location,
            args.start_random
        )
    if args.save:
    ...
    if not(is_human_rat):
        p1 = mp.Process(target=player, args=("rat", args.rat, q1_in, q1_out, q1_quit,
                        width, height, args.preparation_time, args.turn_time,))
        p1.start()
    else:
        q1_out.put("human")
    if not(is_human_python):
        p2 = mp.Process(target=player, args=("python", args.python, q2_in, q2_out,
                        q2_quit, width, height, args.preparation_time, args.turn_time,))
        p2.start()
    else:
        q2_out.put("human")
    ...
    # Prepare save match
    if args.save_match:
        savematch_dir = "saves" + os.path.sep + \
        ...
        for f in [savematch_p1, savematch_p2]:
            f.write("turn_nb = 0\n\n")
            f.write("def preprocessing (maze_map, maze_width, maze_height, "
                    "player_location, opponent_location, pieces_of_cheese, time_allowed) :\n")
            f.write("    pass\n\n")
            f.write("def turn (maze_map, maze_width, maze_height, "
                    "player_location, opponent_location, player_score, opponent_score, "
        ...
        for cell in range(width * height):
            ...
            savematch_maze.write(
                str(maze[(x, y)][(x, y+1)] if (x, y+1) in maze[(x, y)] else 0) + " ")
            savematch_maze.write(
                str(maze[(x, y)][(x, y-1)] if (x, y-1) in maze[(x, y)] else 0) + " ")
            savematch_maze.write(
                str(maze[(x, y)][(x-1, y)] if (x-1, y) in maze[(x, y)] else 0) + " ")
            savematch_maze.write(str(maze[(x, y)][(x+1, y)] if (x+1, y)
                                 in maze[(x, y)] else 0) + "\n")
    ...
    if not(args.nodrawing):
        q_render_quit = Queue()
        draw = Thread(target=run, args=(maze, width, height, q_render, q_render_in, q_render_quit,
                                        p1name, p2name, q1_out, q2_out, is_human_rat,
                                        is_human_python, q_info, pieces_of_cheese,
                                        player1_location, player2_location, args.rat != "",
    ...
    while 1:
        ...
        q_render.put((pieces_of_cheese.copy(), player1_location, player2_location,
                     score1, score2, moves1, moves2, miss1, miss2, stucks1, stucks2))

        # Check if one of the players won
        if args.rat != "" and args.python != "":
            if score1 == score2 and score1 >= args.pieces / 2:
                send_info("The Rat(" + p1name + ") and the Python (" + p2name +
                          ") got the same number of pieces of cheese!", q_info)
                break
        ...
        if args.save:
            savefile.write(
                "# turn "+str(turns) + " rat_location then python_location then pieces_of_cheese "
                "then rat_decision then python_decision\n")
            savefile.write(str(player1_location) + "\n")
            savefile.write(str(player2_location) + "\n")
            savefile.write(str(pieces_of_cheese) + "\n")
        ...
        player1_location, player2_location, stuck1, stuck2, moves1, moves2, miss1, miss2 = move(
            decision1, decision2, maze, player1_location, player2_location, stuck1, stuck2,
            moves1, moves2, miss1, miss2)
    ...
    stats = {
        "win_rat": win1, "win_python": win2, "score_rat": score1, "score_python": score2,
        "moves_rat": moves1, "moves_python": moves2, "miss_rat": miss1, "miss_python": miss2,
        "stucks_rat": stucks1, "stucks_python": stucks2, "prep_time_rat": p1_prep_delay,
        "prep_time_python": p2_prep_delay, "turn_time_rat": p1_turn_delay,
    ...
    if not(args.nodrawing):
        if not(args.save_images):
            ...
            image_icon = pygame.image.load(
                "resources" + os.path.sep + "various" + os.path.sep + "pyrat.ico")
            ...
            if args.fullscreen:
                ...
                screen = pygame.display.set_mode(
                    (args.window_width, args.window_height), pygame.RESIZABLE)
        else:
