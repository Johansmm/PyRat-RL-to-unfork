...
def image_of_maze(maze, tiles, image_tile, image_wall, image_corner, image_mud, offset_x, offset_y, scale, width, height, screen, window_height):
    ...
    for i in range(width):
        for j in range(height):
            screen.blit(image_tile[tiles[i][j]], (offset_x + scale *
                        i, window_height - offset_y - scale * (j+1)))
    for i in range(width):
        for j in range(height):
            if not((i-1, j) in maze[(i, j)]):
                pass
            elif maze[(i, j)][(i-1, j)] > 1:
                screen.blit(image_mud, (offset_x + scale * i - scale/2,
                            window_height - offset_y - scale * (j+1)))
            if not((i, j+1) in maze[(i, j)]):
                pass
            elif maze[(i, j)][(i, j+1)] > 1:
                screen.blit(pygame.transform.rotate(image_mud, 270), (offset_x +
                            scale * i, window_height - offset_y - scale * (j+1) - scale/2))
    for i in range(width):
        for j in range(height):
            if not((i-1, j) in maze[(i, j)]):
                screen.blit(image_wall, (offset_x + scale * i - scale /
                            2, window_height - offset_y - scale * (j+1)))
            if not((i, j+1) in maze[(i, j)]):
                screen.blit(pygame.transform.rotate(image_wall, 270), (offset_x +
                            scale * i, window_height - offset_y - scale * (j+1) - scale/2))
    for i in range(width):
        screen.blit(pygame.transform.rotate(image_wall, 270),
                    (offset_x + scale * i, window_height - offset_y - scale/2))
    for j in range(height):
        screen.blit(image_wall, (offset_x + scale * width - scale /
                    2, window_height - offset_y - scale * (j+1)))
    for i in range(width+1):
        for j in range(height+1):
            ...
            if vert and horiz or count == 1:
                screen.blit(image_corner, (offset_x + scale * i - scale/2,
                            window_height - offset_y - scale * j - scale/2))
...
def draw_text(text, font, color, max_size, index_size, x, y, screen):
    global font_sizes
    font = pygame.font.Font("resources" + os.path.sep + "fonts" +
                            os.path.sep + font + ".ttf", font_sizes[index_size])
...
def draw_scores(p1name, score1, image1, p2name, score2, image2, window_width, window_height, screen, player1_is_alive, player2_is_alive, moves1, miss1, moves2, miss2, stuck1, stuck2):
    if player1_is_alive:
        draw_text("Score: "+str(score1), "Kalam-Bold", (50, 50, 50), window_width /
                  6, 0, int(window_width / 12), window_width / 3 + 50, screen)
        draw_text(p1name, "Kalam-Bold", (50, 50, 50), window_width / 6,
                  5, int(window_width / 12), window_width / 3, screen)
        draw_text("Moves: " + str(moves1), "Kalam-Regular", (2, 118, 137), window_width /
                  6, 1, int(window_width / 12), window_width / 3 + 150, screen)
        draw_text("Miss: " + str(miss1), "Kalam-Regular", (229, 35, 64), window_width /
                  6, 1, int(window_width / 12), window_width / 3 + 180, screen)
        draw_text("Mud: " + str(stuck1), "Kalam-Regular", (229, 35, 64), window_width /
                  6, 1, int(window_width / 12), window_width / 3 + 210, screen)
    if player2_is_alive:
        draw_text("Score: "+str(score2), "Kalam-Bold", (50, 50, 50), window_width /
                  6, 2, int(11 * window_width / 12), window_width / 3 + 50, screen)
        draw_text(p2name, "Kalam-Bold", (50, 50, 50), window_width / 6, 6,
                  int(11 * window_width / 12), window_width / 3, screen)
        draw_text("Moves: " + str(moves2), "Kalam-Regular", (2, 118, 137), window_width /
                  6, 3, int(11 * window_width / 12), window_width / 3 + 150, screen)
        draw_text("Miss: " + str(miss2), "Kalam-Regular", (229, 35, 64), window_width /
                  6, 3, int(11 * window_width / 12), window_width / 3 + 180, screen)
        draw_text("Mud: " + str(stuck2), "Kalam-Regular", (229, 35, 64), window_width /
                  6, 3, int(11 * window_width / 12), window_width / 3 + 210, screen)
...
def init_coords_and_images(width, height, player1_is_alive, player2_is_alive, window_width, window_height):
    ...
    image_background = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "illustrations" + os.path.sep + "background.jpg"), (window_width, window_height))
    image_cheese = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "cheese.png"), (scale, scale))
    image_corner = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "corner.png"), (scale, scale))
    image_moving_python = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "movingPython.png"), (scale, scale))
    image_moving_rat = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements/movingRat.png"), (scale, scale))
    image_python = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "python.png"), (scale, scale))
    image_rat = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "rat.png"), (scale, scale))
    image_wall = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "wall.png"), (scale, scale))
    image_mud = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "game_elements" + os.path.sep + "mud.png"), (scale, scale))
    image_portrait_python = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "illustrations" + os.path.sep + "python_left.png"), (scale_portrait_w, scale_portrait_h))
    image_portrait_rat = pygame.transform.smoothscale(pygame.image.load(
        "resources" + os.path.sep + "illustrations" + os.path.sep + "rat.png"), (scale_portrait_w, scale_portrait_h))
    image_tile = []
    for i in range(10):
        image_tile.append(pygame.transform.smoothscale(pygame.image.load(
            "resources" + os.path.sep + "game_elements" + os.path.sep + "tile"+str(i+1)+".png"), (scale, scale)))
    tiles = []
    for i in range(width):
        tiles.append([])
        for j in range(height):
...
def build_background(screen, maze, tiles, image_background, image_tile, image_wall, image_corner, image_mud, offset_x, offset_y, width, height, window_width, window_height, image_portrait_rat, image_portrait_python, scale, player1_is_alive, player2_is_alive):
    ...
    image_of_maze(maze, tiles, image_tile, image_wall, image_corner, image_mud,
                  offset_x, offset_y, scale, width, height, maze_image, window_height)

    if player1_is_alive:
        maze_image.blit(image_portrait_rat, (int(window_width / 12 -
                        image_portrait_python.get_rect().width / 2), 100))
    if player2_is_alive:
        maze_image.blit(image_portrait_python, (int(window_width * 11 / 12 -
                        image_portrait_python.get_rect().width / 2), 100))
    return maze_image
...
def run(maze, width, height, q, q_render_in, q_quit, p1name, p2name, q1_out, q2_out, is_human_rat, is_human_python, q_info, pieces_of_cheese, player1_location, player2_location, player1_is_alive, player2_is_alive, screen, infoObject):
    ...
    try:
        ...
        j0.init()
        print('Enabled joystick: ' + j0.get_name() + ' with ' +
              str(j0.get_numaxes()) + ' axes', file=sys.stderr)
        j1 = pygame.joystick.Joystick(1)
        j1.init()
        print('Enabled joystick: ' + j1.get_name() + ' with ' +
              str(j1.get_numaxes()) + ' axes', file=sys.stderr)
    except pygame.error:
        ()

    debug("Building background image", 2)
    maze_image = build_background(screen, maze, tiles, image_background, image_tile, image_wall, image_corner, image_mud, offset_x, offset_y,
                                  width, height, window_width, window_height, image_portrait_rat, image_portrait_python, scale, player1_is_alive, player2_is_alive)
    ...
    while q_quit.empty() or (args.desactivate_animations and not(q.empty())):
        ...
        if (args.desactivate_animations and not(q.empty())) or not(args.desactivate_animations):
                ...
                while not(q.empty()):
                    ...
                    if not(args.desactivate_animations):
                        if nnew_player1_location != new_player1_location:
                            time_to_go1 = pygame.time.get_ticks() + turn_time * \
                                maze[new_player1_location][nnew_player1_location]
                            player1_location = new_player1_location
                        if nnew_player2_location != new_player2_location:
                            player2_location = new_player2_location
                            time_to_go2 = pygame.time.get_ticks() + turn_time * \
                                maze[new_player2_location][nnew_player2_location]
            ...
            draw_pieces_of_cheese(pieces_of_cheese, image_cheese, offset_x,
                                  offset_y, scale, width, height, screen, window_height)
            ...
            if not(args.desactivate_animations):
                ...
                draw_players_animate(player1_draw_location, player2_draw_location, image1,
                                     image2, offset_x, offset_y, scale, width, height, screen, window_height)
            else:  # if desactivate_animations
                draw_players(player1_location, player2_location, image_rat, image_python,
                             offset_x, offset_y, scale, width, height, screen, window_height)
            draw_scores(p1name, score1, image_portrait_rat, p2name, score2, image_portrait_python, window_width,
                        window_height, screen, player1_is_alive, player2_is_alive, moves1, miss1, moves2, miss2, stuck1, stuck2)
            if not(q_info.empty()):
                text_info = q_info.get()
            if text_info != "":
                draw_text(text_info, "Kalam-Bold", (50, 50, 50),
                          window_width, 4, window_width // 2, 25, screen)
            ...
            if args.save_images:
                pygame.image.save(screen, "output_images" + os.path.sep +
                                  "image" + str(d)[1:] + ".png")
                d = d + 1
        else:
            clock.tick(60)
